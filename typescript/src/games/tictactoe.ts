...
const sum = (arr: number[]): number => { return arr.reduce((x, y) => x + y, 0) };
...
const _NoPossibleMove = (board: Board): boolean => {
    const indices: Idx[] = [0, 1, 2];
    return sum(indices.map(i => sum(indices.map(
        j => tokenMap[board[i][j]] === 0 ? 1 : 0
    )))) <= 0;
};
...
const determineWinner = (game: Game): MaybePlayer => {
    ...
    if (sumRow1 == 3) { return player1; };
    if (sumRow2 == 3) { return player2; };

    const [sumCol1, sumCol2]: [number, number] = sumBoardCol(board)
    if (sumCol1 == 3) { return player1; };
    if (sumCol2 == 3) { return player2; };

    const [sumDiag1, sumDiag2]: [number, number] = sumBoardDiag(board)
    if (sumDiag1 == 3) { return player1; };
    if (sumDiag2 == 3) { return player2; };

    if (_NoPossibleMove(game.board)) { return -1 };
...
const getLegalMoves = (game: Game): Move[] => {
    if (determineWinner(game)) { return [] };
...
const initGame = (board?: Board, playerIdx?: PlayerIdx): Game => {
    ...
    const board_: Board = (board !== undefined) ? board : [
        [" ", " ", " "],
        [" ", " ", " "],
        [" ", " ", " "]];
    const game: Game = {
...
export const TicTacToeEngine: BaseEngine = {
...
let row1: Row = [" ", "o", "o"];
let row2: Row = ["o", " ", "x"];
let board: Board = [row0, row1, row2];
let players: Players = [1, 2];
let playerIdx: PlayerIdx = 0;
let game: Game = {
    players: players,
    playerIdx: playerIdx,
    board: board,
...
console.log(game);
console.log(sumBoardRow(board));
console.log(sumBoardDiag(board));
console.log(sumBoardDiag(board));
console.log(determineWinner(game));
console.log(getLegalMoves(game));

let move: Move = { i: 0, j: 2 };
let game2 = makeMove(game, move);
console.log(game2.board);
console.log(determineWinner(game2));
console.log(typeof TicTacToeEngine) */